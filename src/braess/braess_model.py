...
class PIGN_rho(nn.Module):
    ...
    def forward(self, x, messages=None):
        ...
        )  # x_channel: (n_samples, T, (N + 1) * N_edges, (N + 1) * N_edges, n_features)
        x_sum = self.f_channel(x_channel)[
            :, :, :, :, 0
        ]  # x_sum: (n_samples, T, (N + 1) * N_edges, (N + 1) * N_edges)
        message_j = self.f_sum(x_sum)
        x_x = torch.concat(
            (rhos[:, :, :, None], message_j), dim=-1
        )  # x_x: (n_samples, T, (N + 1) * N_edges, 2)
        outs = self.f_x(x_x)[:, :, :, 0]
        return outs
...
class PIGN_V(nn.Module):
    ...
    def forward(self, x, messages=None):
        ...
        x_channel = torch.from_numpy(messages).to(
            self.f_channel.device
        )  # x_channel: (n_samples, T + 1, (N + 2) * N_edges, (N + 2) * N_edges, n_features)
        x_sum = self.f_channel(x_channel)[
            :, :, :, :, 0
        ]  # x_sum: (n_samples, T + 1, (N + 2) * N_edges, (N + 2) * N_edges)
        message_j = self.f_sum(x_sum)
        x_x = torch.concat(
            (Vs[:, :, :, None], message_j), dim=-1
        )  # x_x: (n_samples, T + 1, (N + 2) * N_edges, 2)
        outs = self.f_x(x_x)[:, :, :, 0]
        return outs
