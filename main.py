...
if __name__ == "__main__":
    ...
    if args.network == "ringroad":
        ...
        for ep in range(20):
            print(f"-------------------{ep}-------------")
            u_message = braess_loader.get_u_from_rho_V_pi(rho_preds, V_preds, pi)
            u_hist.append(u_message)
            u_message = np.array(u_hist).mean(axis=0)

            rho_preds, rho_loss = run_rho(
                braess_loader, u_message, rho_preds, beta, f_args, config
            )
            V_preds, V_loss = run_V(
                braess_loader, u_message, rho_preds, pi, f_args, config
            )
